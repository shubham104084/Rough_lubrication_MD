...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 13 15:05:07 2025
...
n_atoms = len(r_bot)+len(r_top)

x_lo = min(min(r_top[:,0]),min(r_bot[:,0]))
x_hi = max(max(r_top[:,0]),max(r_bot[:,0]))
y_lo = min(min(r_top[:,1]),min(r_bot[:,1]))
y_hi = max(max(r_top[:,1]),max(r_bot[:,1]))
z_lo = min(min(r_top[:,2]),min(r_bot[:,2]))-10.0
z_hi = max(max(r_top[:,2]),max(r_bot[:,2]))+10.0
...
pos_cat = np.concatenate((r_top, r_bot), axis=0)

# --- write file --------------------------------------------------------
filename='config.data'
with open(filename, "w") as f: 
    f.write(f"LAMMPS data file using Python\n\n")
    f.write(f"{n_atoms:d} atoms\n")
    f.write(f"{1} atom types\n")
    f.write("\n")
    
    f.write(f"{x_lo: .16g} {x_hi: .16g} xlo xhi\n")
    f.write(f"{y_lo: .16g} {y_hi: .16g} ylo yhi\n")
    f.write(f"{z_lo: .16g} {z_hi: .16g} zlo zhi\n")
    f.write("\n")
    ...
    for i in range(n_atoms):
        aid = i + 1
        f.write(f"{aid:d} {1} {1} {0.0} {pos_cat[i,0]: .16g} {pos_cat[i,1]: .16g} {pos_cat[i,2]: .16g}\n")
    f.write("\n")
    
   

    # Return a tiny summary (useful in scripts)
